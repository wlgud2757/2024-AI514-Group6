...
def graph_reader_from_dat(input_path):
    """
    Function to read an edge list from a .dat file and transform it to a networkx graph object.
    """
    edges = pd.read_csv(input_path, delimiter='\s+', header=None)  # Assuming the .dat file is space-separated
    graph = nx.from_edgelist(edges.values.tolist())
    return graph
